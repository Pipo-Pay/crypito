...
class MinerThread(QtCore.QThread):
    ...
    def exibir_animacao_mineracao(self):
        ...
        for _ in tqdm(range(passos), desc="⛏️ Minerando... 💰"):
            time.sleep(0)  # Cada passo dura 1 segundo
...
class CryptoClient(QtWidgets.QWidget):
    ...
    def send_balance(self):
        # Extrair informações dos campos de forma otimizada
        sender_address = self.sender_input.text().strip()
        private_key = self.private_key_input.text().strip()
        recipient = self.recipient_input.text().strip()
        amount = self.send_amount_input.text().strip()

        # Validações simplificadas
        if not sender_address or not private_key or not recipient or not amount:
            self.balance_label.setText('Todos os campos são obrigatórios!')
            return
        ...
        try:
            ...
            self.balance_label.setText('Valor inválido para a transação.')
            return
        ...
        # Comunicação com o servidor usando uma abordagem assíncrona para não travar a interface
        try:
            response = requests.post('http://127.0.0.1/transfer', json=transaction_data)

            # Resposta otimizada para clareza
            if response.status_code == 200:
                transaction_info = response.json()
                self.balance_label.setText(f'Transação realizada com sucesso: {transaction_info.get("message", "Sem mensagem")}')
            else:
                self.balance_label.setText(f'Erro {response.status_code}: {response.text}')
        except requests.exceptions.RequestException as e:
            self.balance_label.setText(f'Erro ao se conectar ao servidor: {e}')

    def start_mining(self, miner_address):
        # Inicia a mineração em segundo plano
        self.miner_thread.start_mining(miner_address)
    ...
    def update_mining_status(self, block_index, previous_hash):
        # Atualiza o status de mineração e exibe o QR Code com os dados
        self.mining_status_label.setText(f'Minerando... Bloco {block_index} - Hash: {previous_hash}')
        qr_data = f"Bloco: {block_index}\nHash Anterior: {previous_hash}"
        self.display_qr_code(qr_data)

    def generate_qr_code(self, index, previous_hash):
        # Gerar QR Code de forma otimizada
        ...
        # Adiciona botão de ampliar
        self.maximize_qr_btn = QPushButton('Ampliar QR Code', self)
        self.maximize_qr_btn.clicked.connect(self.maximize_qr_code)
        self.layout.addWidget(self.maximize_qr_btn)

    def maximize_qr_code(self):
        # Exibe o QR Code ampliado
        qr_data = self.mining_status_label.text()
        self.qr_code_window = QrCodeViewer(qr_data)
        self.qr_code_window.show()

    def display_qr_code(self, qr_data):
        # Método para exibir QR Code (caso precise de um código separado)
        qr_image = qrcode.make(qr_data)
        qr_bytes = BytesIO()
        qr_image.save(qr_bytes, format='PNG')
        qr_pixmap = QPixmap()
